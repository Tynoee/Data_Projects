# 📊 Data Analytics Portfolio — Tinotenda Chidume

I'm a Junior Data Analyst with a background in data engineering, business intelligence, and analytics. I enjoy turning messy, raw datasets into clean pipelines and clear insights that actually help people make better decisions.

This portfolio covers projects across **SQL, Python, Excel, and Tableau** — from ETL pipelines and EDA to interactive dashboards and statistical process control.

---

## 🛠️ Tech Stack

**Languages & Analysis**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)

**Visualisation & BI**

![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoftexcel&logoColor=white)

**Engineering & Workflow**

![ETL](https://img.shields.io/badge/ETL_Pipelines-informational?style=flat)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)

---

## 📁 Projects

### 🐍 Python

| Project | Description | Key Skills |
|---|---|---|
| [Bank Customer EDA](./Bank%20Customer%20Data%20Prep) | Cleaned 10,000+ customer records, engineered features, and identified churn drivers across segments | EDA, Feature Engineering, Pandas |
| [Airbnb Paris Analysis](./Airbnb%20Listings%20Analysis) | Analyzed 250,000+ listings to uncover pricing patterns, host growth trends, and regulatory impacts | Data Analysis, Visualisation |
| [Walmart ETL Pipeline](./Walmart%20Holiday%20Sales%20Data%20Pipeline) | Built a modular ETL pipeline integrating PostgreSQL and Parquet sources with validation and export | ETL, Data Engineering, Pandas |

---

### 🗄️ SQL

| Project | Description | Key Skills |
|---|---|---|
| [Hospital Analytics](./Hospital%20Analytics) | Queried 75,000+ patient records to analyze encounter trends, readmissions, and cost gaps | Subqueries, Time Series, Aggregations |
| [B2B Sales Pipeline](./Sales%20Pipeline%20Analysis) | Analyzed 8,800+ opportunities across agents and products to evaluate win rates and deal lifecycles | Window Functions, Joins |
| [Manufacturing Process Control](./Evaluate%20Manufacturing%20Process) | Applied window functions to compute statistical control limits and built an out-of-tolerance alert system | Statistical Analysis, ETL |
| [Student Mental Health](./Student%20Mental%20Health%20Analysis) | Explored PHQ-9, SCS, and ASISS wellbeing metrics across international student cohorts | Aggregations, Filtering |
| [COVID-19 Global Analysis](./COVID-19%20Data%20Analysis%20%26%20Visualization) | Analyzed global infection, death, and vaccination trends with an interactive Tableau dashboard | SQL + Tableau, Data Cleaning |

---

### 📊 Excel & BI

| Project | Description | Key Skills |
|---|---|---|
| [Global Electronics Retailer](./Global%20Electronics%20Retailer) | Modeled 62,000+ records using star schema; built DAX measures for Revenue, AOV, and Delivery Time | Star Schema, DAX, Power Pivot |
| [Customer Complaint Tracker](./Customer%20Complaint%20Tracking) | Tracked 62,500+ banking complaints with dynamic dashboards and trend analysis | PivotTables, Dashboard Design |
| [Manufacturing Downtime Analysis](./Manufacturing%20Downtime%20Analysis) | Used Pareto analysis and lookup functions to isolate downtime causes and identify low-efficiency operators | Pareto, Operational Analytics |
| [NRM Case Study](./Net%20Revenue%20Management) | Analyzed Market Share and YoY Growth KPIs; built forecasting models to support pricing strategy | Forecasting, Strategic Analysis |

---

## 📬 Get in Touch

I'm currently open to Data Analyst and Junior Data Engineer roles.

- 📧 tinotendachidume@gmail.com
- 💼 [LinkedIn](https://www.linkedin.com/in/tinotenda-tylee-chidume/)
